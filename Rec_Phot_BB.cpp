...
double Rec_Phot_BB_SH::g_phot_ion_lim(double nu)
{ return sig_phot_ion_lim(nu)/sig_phot_ion_Kramers(nu); }
...
{
    return x*x/one_minus_exp_mx(x)*exp(Rec_Phot_BB_xi-x)
...
{ 
    ...
    return x*x/one_minus_exp_mx(x)*exp((xi-x)/rho)
            *ptr_to_Rec_Phot_BB_SH->sig_phot_ion_lim(x*Rec_Phot_BB_T_g/const_h_kb);
}
...
{ 
    ...
    return (x-xi-1.5*rho)*x*x/one_minus_exp_mx(x)*exp((xi-x)/rho)
                    *ptr_to_Rec_Phot_BB_SH->sig_phot_ion_lim(x*Rec_Phot_BB_T_g/const_h_kb);
}
...
void Rec_Phot_BB_SH_QSP::init_parallel(int nv, int lv, int Z, double Np, int mflag)
{
    ...
    if(lv>=lmin_spline) spline_is_allocated=0;
    else
    {
...
{ 
    ...
    return x*x/one_minus_exp_mx(x)*exp(V->xi-x)
...
{ 
    ...
    return x*x/one_minus_exp_mx(x)*exp((V->xi-x)/V->rho)
            *V->SH_QSP_ptr->sig_phot_ion_lim(x*V->T_g/const_h_kb);
}
...
{ 
    ...
    return (x-V->xi-1.5*V->rho)*x*x/one_minus_exp_mx(x)*exp((V->xi-x)/V->rho)
                       *V->SH_QSP_ptr->sig_phot_ion_lim(x*V->T_g/const_h_kb);
}
